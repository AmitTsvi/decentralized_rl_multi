...
#include "open_spiel/games/coop_box_pushing_serial.h"
...
namespace coop_box_pushing_serial {
namespace {
...
const GameType kGameType{
    /*short_name=*/"coop_box_pushing_serial",
    /*long_name=*/"Cooperative Box Pushing Serial",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new CoopBoxPushingSerialGame(params));
}
...
CoopBoxPushingSerialState::CoopBoxPushingSerialState(std::shared_ptr<const Game> game,
...
void CoopBoxPushingSerialState::SetPlayer(std::pair<int, int> coord, Player player,
...
char CoopBoxPushingSerialState::field(std::pair<int, int> coord) const {
  return field_[coord.first * kCols + coord.second];
}

void CoopBoxPushingSerialState::DoApplyActions(const std::vector<Action>& actions) {
...
bool CoopBoxPushingSerialState::InBounds(std::pair<int, int> coord) const {
  return (coord.first >= 0 && coord.second >= 0 && coord.first < kRows &&
          coord.second < kCols);
}

void CoopBoxPushingSerialState::MoveForward(Player player) {
...
void CoopBoxPushingSerialState::AddReward(double reward) {
  reward_ += reward;
...
void CoopBoxPushingSerialState::ResolveMoves() {
  ...
  if (player0_try || player1_try) {
    ...
    if (InBounds(next_coords[0]) && InBounds(next_coords[1]) &&
        ...
        if ((player0_try && lock_status_[1]) || (player1_try && lock_status_[0])){
...
void CoopBoxPushingSerialState::DoApplyAction(Action action) {
...
std::vector<Action> CoopBoxPushingSerialState::LegalActions(Player player) const {
...
ActionsAndProbs CoopBoxPushingSerialState::ChanceOutcomes() const {
...
std::string CoopBoxPushingSerialState::ToString() const {
  std::string result = "";
...
ObservationType CoopBoxPushingSerialState::PartialObservation(Player player) const {
...
std::string CoopBoxPushingSerialState::ObservationString(Player player) const {
  SPIEL_CHECK_GE(player, 0);
...
bool CoopBoxPushingSerialState::IsTerminal() const {
  return (total_moves_ >= horizon_ || win_);
}

std::vector<double> CoopBoxPushingSerialState::Returns() const {
  // Cooperative game: all players get same reward.
  return {total_rewards_, total_rewards_};
}

std::vector<double> CoopBoxPushingSerialState::Rewards() const {
  // Cooperative game: all players get same reward.
  return {reward_, reward_};
}

bool CoopBoxPushingSerialState::SameAsPlayer(std::pair<int, int> coord,
                                       Player player) const {
  return coord == player_coords_[player];
}

int CoopBoxPushingSerialState::ObservationPlane(std::pair<int, int> coord,
                                          Player player) const {
  int plane = 0;
  switch (field(coord)) {
    case kField:
...
void CoopBoxPushingSerialState::ObservationTensor(Player player,
...
std::unique_ptr<State> CoopBoxPushingSerialState::Clone() const {
  return std::unique_ptr<State>(new CoopBoxPushingSerialState(*this));
}

CoopBoxPushingSerialGame::CoopBoxPushingSerialGame(const GameParameters& params)
    : SimMoveGame(kGameType, params),
      horizon_(ParameterValue<int>("horizon")),
      fully_observable_(ParameterValue<bool>("fully_observable")) {}

std::vector<int> CoopBoxPushingSerialGame::ObservationTensorShape() const {
  if (fully_observable_) {
...
int CoopBoxPushingSerialGame::NumDistinctActions() const {
  return kNumDistinctActions;
}

int CoopBoxPushingSerialGame::NumPlayers() const { return kNumPlayers; }

std::unique_ptr<State> CoopBoxPushingSerialGame::NewInitialState() const {
  std::unique_ptr<State> state(
      new CoopBoxPushingSerialState(shared_from_this(), horizon_, fully_observable_));
  return state;
}
...
// episode length.
double CoopBoxPushingSerialGame::MaxUtility() const {
  return MaxGameLength() * NumPlayers() * (kBigBoxReward + kDelayPenalty);
}

double CoopBoxPushingSerialGame::MinUtility() const {
