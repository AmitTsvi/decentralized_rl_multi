...
class VisualEnvManager(EnvManager):
    ...
    def save_image(self, fname, i, ret, frame, bids_t, s_bids_t):
        fig = plt.figure()
        ...
        s_ids, s_bids = zip(*s_bids_t)
        ax2.bar(s_ids, s_bids, align='center', alpha=0.5)
        ax2.set_xticks(range(len(s_ids)), map(int, s_ids))
        ax2.xaxis.set_major_locator(MaxNLocator(integer=True))
        ax2.set_ylim(0.0, 1.0)
        ax2.set_ylabel('Bid')
    ...
    def save_video(self, epoch, test_example, bids, s_bids, ret, frames, ext=''):
        for i, frame in enumerate(frames):
            ...
            s_ids = sorted(s_bids.keys())
            s_bids_t = [(s_id, s_bids[s_id][i]) for s_id in s_ids]
            self.save_image(fname, i, ret, frame, bids_t, s_bids_t)
